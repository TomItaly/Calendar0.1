...
export class CalendarPage implements OnInit, OnDestroy {
  ...
  loadData(id, idPresent?) {
    ...
    this.calendarService.getEvents()
      ...
      this.displayNotifications(id);
      this.reminderNotifications(id);
  }

  displayNotifications(id) {
    let iCounter = 0;
      ...
      .subscribe((data: any) => {
        if (data && data.events.length > 0) {
          ...
          for (let i = 0; i < value.length; i++) {
            iCounter = i + 1;
            const text = `Dziś od ${moment(value[i].start).format('hh:mm a')} 
      ...
      .subscribe((data: any) => {
        if (data && data.events.length > 0) {
          ...
          for (let j = 0; j < value.length; j++) {
            ...
            const text = `Masz nadchodzące wydarzenie o ${moment(value[j].start).format('hh:mm a')}`;
            this.localNotification.schedule({
              id: jCounter,
              title: 'Przypomnienie',
              text: text,
